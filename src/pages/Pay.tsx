      ...
      <div className="w-full max-w-5xl space-y-4">
          ...
          >
            ...
            >
              ...
              <div className="flex justify-center p-4 bg-[#0f172a] rounded-xl">
                <QRCodeCanvas
                  ref={qrRef}
                  value={`https://starkzappypay.vercel.app/pay/@${displayName}`}
                  size={200}
                  bgColor="#0f172a"
                  fgColor="#a78bfa"
                  level="M"
              ...
              <p className="text-slate-500 text-xs text-center break-all">
                  starkzappypay.vercel.app/pay/@{displayName}
              </p>
              <button
                onClick={() => {
        ...
        <div className="bg-slate-900 rounded-2xl border border-slate-800 p-6 text-center space-y-3">
          ...
          <div className="space-y-1">
            ...
            {tipMessage && (
              <p className="text-white font-semibold text-lg mt-1.5">
                "{tipMessage}"
              </p>
            )}
          </div>
          ...
          >
            <div className="flex items-center justify-between gap-2">
              <div>
                ...
                <p
                  className={`text-xs mt-0.5 ${goalPct >= 100 ? "text-emerald-400" : "text-slate-500"}`}
                >
                  {goalPct >= 100
                    ? "Goal reached — tips still welcome!"
                    : `${Math.round(goalPct)}% funded`}
                </p>
              </div>
              <div className="text-right flex-shrink-0">
                <p
                  className={`text-sm font-bold ${goalPct >= 100 ? "text-emerald-400" : "text-violet-400"}`}
                >
            ...
            {!connected ? (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  {[
                    ...
                    <div
                      key={f.text}
                      className="bg-slate-800/50 rounded-xl p-2.5 space-y-1"
                    >
              ...
              <>
                ...
                <div className="bg-slate-800/60 border border-slate-700 rounded-xl p-4 space-y-3">
                  ...
                  <div className="flex gap-2 bg-amber-500/10 border border-amber-500/20 rounded-lg px-3 py-2.5">
                    ...
                    <p className="text-amber-300/80 text-xs leading-relaxed">
                      ...
                      This is your starkzappypay wallet. Send STRK, USDC, or ETH
                      here from your main wallet first.
                    </p>
                  </div>
                </div>
                  ...
                  <div className="bg-violet-500/10 border border-violet-500/20 rounded-xl px-4 py-2.5 flex items-center gap-2">
                    ...
                    <p className="text-violet-300 text-sm">
                      ...
                      <span className="font-semibold">
                        {tipperCount} time{tipperCount !== 1 ? "s" : ""}
                      </span>{" "}
                      before!
                    </p>
                  </div>
                  ...
                  <div className="bg-violet-500/10 border border-violet-500/20 rounded-xl px-4 py-3 space-y-1">
                    ...
                    {fetchingQuote ? (
                      ...
                      <p className="text-slate-400 text-xs">
                        Quote unavailable — will send {selectedToken.symbol}{" "}
                        directly
                      </p>
                    )}
                  </div>
                ...
                {swapGasError && (
                  ...
                  >
                    Send {amount || "0"} {selectedToken.symbol} directly (skip
                    swap)
                  </button>
                )}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-white text-sm font-medium truncate">
                          {s.tipper_name || "Anonymous"}
                        </p>
                        <p className="text-slate-500 text-xs">
                          {s.tip_count} tip{s.tip_count !== 1 ? "s" : ""}
                          {s.total_preferred > 0 &&
                            ` · ${s.total_preferred.toFixed(s.total_preferred % 1 === 0 ? 0 : 2)} ${preferredTokenSymbol}`}
                        </p>
                      </div>
                    </div>
                  ))}
              ...
              {wallTab === "recent" && (
                <>
                  <div className="space-y-3">
                    {tips
                      .slice(
                        tipsPage * TIPS_PER_PAGE,
                        (tipsPage + 1) * TIPS_PER_PAGE,
                      )
                      .map((tip, i) => (
                        <a
                          key={tip.id ?? i}
                          href={
                            tip.tx_hash
                              ? `https://voyager.online/tx/${tip.tx_hash}`
                              : undefined
                          }
                          target="_blank"
                            ...
                            <div className="flex items-center justify-between gap-2">
                              <div className="flex items-center gap-1.5 min-w-0">
                                ...
                                {tip.tx_hash && (
                                  <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    className="w-3 h-3 text-slate-600 group-hover:text-violet-400 flex-shrink-0 transition-colors"
                                    fill="none"
                                    viewBox="0 0 24 24"
                                    stroke="currentColor"
                                  >
                  ...
                  {tips.length > TIPS_PER_PAGE && (
                    <div className="flex items-center justify-between pt-1 border-t border-slate-800">
                      ...
                      <span className="text-slate-600 text-xs">
                        {tipsPage + 1} /{" "}
                        {Math.ceil(tips.length / TIPS_PER_PAGE)}
                      </span>
                      <button
                        onClick={() =>
                          setTipsPage((p) =>
                            Math.min(
                              Math.ceil(tips.length / TIPS_PER_PAGE) - 1,
                              p + 1,
                            ),
                          )
                        }
